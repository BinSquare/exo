...
use executor::{
    ...
    SecretBackendChoice, SpritesBackendSpec, StartSandboxRequest, ToolRequest, ToolRuntime,
    TypeScriptHarness, TypeScriptHarnessConfig, Uuid7, VercelBackendSpec,
    attached_conversation_sandbox, default_aws_agentcore_image, default_daytona_image,
        ...
        Commands::Conversation { command } => match command {
            ...
            ConversationCommands::Sandbox { command } => match command {
                ...
                ConversationSandboxCommands::Restore {
                    ...
                    let snapshot_id = Uuid7::from_str(&snapshot_id)
                        .map_err(|error| anyhow!("invalid snapshot id: {error}"))?;
                    let conversation =
                        must_get_conversation(harness.as_ref(), &agent, &conversation).await?;
                    conversation
                        .exoharness_handle()
                    ...
                    if json {
                        ...
                        println!("restored sandbox {sandbox_id} from {snapshot_id}");
                    }
                }
                ConversationSandboxCommands::Stop {
                    agent,
                    conversation,
                    sandbox_id,
                    json,
    ...
    match command {
        ...
        Commands::Conversation { command } => match command {
            ...
            ConversationCommands::Sandbox { command } => match command {
                ...
                | ConversationSandboxCommands::Stop { agent, .. }
