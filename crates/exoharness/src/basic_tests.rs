...
#[tokio::test(flavor = "current_thread")]
async fn attached_sandbox_snapshot_restores_as_owned_after_detach() {
    let tempdir = TempDir::new().expect("tempdir");
    let backend = Arc::new(AttachmentTestBackend::default());
    backend
        .external_resource_exists
    ...
    let agent = harness
        ...
        .expect("agent");
    let agent_id = agent.record().id;
    let original_conversation = agent
        .new_conversation(NewConversationRequest::default())
        .await
        .expect("conversation");
    let conversation_id = original_conversation.record().id;
    let sandbox_id = original_conversation
        .attach_sandbox(AttachSandboxRequest {
    ...
    // Snapshot through a new harness instance, just as separate CLI
    // invocations do in the Harbor integration. The live attachment must be
    // reacquired from its durable sandbox record before it can be captured.
    let reloaded = BasicExoHarness::new(attachment_test_config(tempdir.path(), backend.clone()))
        .await
        .expect("reloaded harness should initialize");
    let reloaded_agent = reloaded
    ...
    let snapshot_id = conversation
        ...
        .await
        .expect("borrowed sandbox should snapshot");
    conversation
        .detach_sandbox(sandbox_id.clone())
        .await
        .expect("borrowed sandbox should detach");
    conversation
        ...
        .await
        .expect("detached snapshot should restore as an owned sandbox");

    // The original harness still caches the borrowed handle. It must observe
    // the durable ownership transition and reacquire the restored sandbox.
    original_conversation
        .snapshot_sandbox(sandbox_id.clone())
        .await
        .expect("stale borrowed handle should be replaced by the restored sandbox");
    original_conversation
        .stop_sandbox(sandbox_id)
        .await
        .expect("restored sandbox should support owned lifecycle operations");
...
impl ManagedSandboxHandle for AttachmentTestHandle {
    ...
    async fn snapshot(&self) -> crate::Result<SnapshotPayload> {
        Ok(SnapshotPayload {
            ...
            bytes: Vec::from("submitted state").into(),
        })
    }
}
